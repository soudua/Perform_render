                              ...
                              <div key={`user-${user.user.user_id}-${userIndex}`} className="mb-4">
                                ...
                                <ul className="list-disc ml-6">
                                  {user.commits.map((commit: any, commitIndex: number) => {
                                    ...
                                    const isShowingDetailed = showDetailedSummary[commitKey];
                                    
                                    return (
                                      <li key={`${user.user.user_id}-${commit.sha}-${commitIndex}`} className="mb-3 text-white">
                                        <div className="mb-1">
                                          <span className="font-mono text-sm text-gray-200">{formatDate(commit.date)}:</span> 
                                        ...
                                        {currentRepoInfo && (
                                          <button
                                            onClick={async () => {
                                              // If detailed summary exists and is currently shown, hide it
                                              if (hasDetailedSummary && isShowingDetailed) {
                                                setShowDetailedSummary(prev => ({ ...prev, [commitKey]: false }));
                                              }
                                              // If detailed summary exists but is hidden, show it
                                              else if (hasDetailedSummary && !isShowingDetailed) {
                                                setShowDetailedSummary(prev => ({ ...prev, [commitKey]: true }));
                                              }
                                              // If no detailed summary exists, generate it and show it
                                              else if (!hasDetailedSummary && !isLoadingDetailed) {
                                            ...
                                            className={`text-xs px-2 py-1 rounded transition-colors ${
                                              hasDetailedSummary && isShowingDetailed
